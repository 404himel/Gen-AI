...
# It also return the information about the database tables such that Who has more than 3.5 cg// the student name/roll it can return
final_query = QuerySQLDataBaseTool(db=db)

# if you wanna a realable output(list out the cse students) then use the specific prompt for the output
chain = write_query | final_query
    ...
    def extract_sql(response_text):
        ...
        for line in lines:
            if line.startswith("SQLQuery:"):
                ...
                query = line.replace("SQLQuery:", "").strip()
